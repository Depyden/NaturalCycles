...
export const setElementStyles = (element: ElementRef) => {
    if (element) {
        if (element.nativeElement.style.fontSize !== "") {
            const multiplyLevel = element.nativeElement.offsetParent.offsetWidth / element.nativeElement.clientWidth;
            const newFontSize = Number(element.nativeElement.style.fontSize.split('px')[0]) * multiplyLevel;
            const newLineHeight = Number(element.nativeElement.style.lineHeight.split('px')[0]) * multiplyLevel;
